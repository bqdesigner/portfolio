// DarkVeil — fundo animado do hero-card.
// Port vanilla do componente React Bits (JS+CSS, ogl). Troca o renderer ogl
// pelo three self-hospedado já usado no hero (mesmo padrão do ColorBends
// anterior); mantém o shader original (rede CPPN), só troca os hooks React
// por setup/cleanup imperativo. Camada WebGL opaca cobre o hero-card inteiro.
...
function initDarkVeil(THREE, container, opts) {
  const {
    hueShift = 0,
    noiseIntensity = 0,
    scanlineIntensity = 0,
  ...
  const handleResize = () => {
    ...
    material.uniforms.uResolution.value.set(w * resolutionScale, h * resolutionScale);
  };
  handleResize();
